...
No exemplo abaixo, mostramos como baixar estimativas de acessibilidade no período de pico em São Paulo referentes ao ano de 2019. Neste exemplo, baixamos tanto as estimativas de acessibilidade por automóvel quanto por transporte público e as unimos em um único `data.frame`. Note que essa função resulta em uma tabela que também traz, automaticamente, os dados de população e de uso do solo.

```{r}
#| warning: false
#| message: false
# baixa dados do AOP de acessibilidade por transporte publico
...
# baixa dados do AOP de acessibilidade por automovel
...
2. O **tipo de atividade** para qual os níveis de acessibilidade foram calculados é indicado pelas letras seguintes, no meio do código. Os dados incluem estimativas de acessibilidade para diversos tipos de atividades:
   ...
   No caso do indicador de acessibilidade passiva (`CMP`), as letras do meio do nome da variável indicam **a qual grupo populacional** os níveis de acessibilidade se referem a:
...
A descrição completa das variáveis também pode ser consultada na documentação da função, rodando em uma sessão do R o comando `?read_access`. As seções a seguir mostram exemplos de visualizações desses dados em forma de mapas e gráficos.
...
# trunca tempos de viagem em 60 minutos
...
# substitui valores Inf por 120
desigualdade_tp[, TMISA := ifelse(is.infinite(TMISA), 120, TMISA)]

# calcula tempo de viagem medio por decil de renda
...
Outra maneira de examinar a desigualdade de acesso a oportunidades é comparar a quantidade de oportunidades acessíveis por diferentes grupos populacionais considerando os mesmos modos de transporte e limites de tempo de viagem. Neste caso, analisamos o indicador de acessibilidade cumulativa ativa, representado por variáveis cujos códigos começam com `CMA` na base de dados do `{aopdata}`. No exemplo abaixo, utilizamos um gráfico *box plot* para comparar a quantidade de empregos acessíveis entre diferentes decis de renda, considerando viagens de transporte público limitadas em 30 minutos de viagem.
...
```{r}
# calcula media de empregos acessiveis em 30 minutos
...
# junta os dados e calcula razao entre acesso por carro e por caminhada
