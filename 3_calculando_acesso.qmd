...
[^ttm_params]: Para mais informações sobre cada um dos parâmetros, consulte a documentação da função em uma sessão de R (com os comandos `?travel_time_matrix()` ou `help(“travel_time_matrix”)`) ou no site do `{r5r}`, [neste link](https://ipeagit.github.io/r5r/reference/travel_time_matrix.html).
