...
Usualmente, arquivos GTFS provenientes de fontes oficiais são utilizados para desenvolver análises e pesquisas que possuem diversos elementos comuns. Visando facilitar a leitura, o processamento e a análise desses dados, a equipe do Projeto Acesso a Oportunidades vêm desenvolvendo o pacote de R `{gtfstools}` [@Herszenhut2022gtfstools][^1], que oferece diversas funções que facilitam a manipulação e a exploração de feeds.
...
Ao final de edições de um objeto GTFS no R, frequentemente vamos querer usar o GTFS manipulado para fazer análises de diferentes tipos. Para isso, é comum que precisemos do arquivo GTFS em formato `.zip` novamente, e não no formato de lista de tabelas dentro do R. O pacote `{gtfstools}` disponibiliza a função `write_gtfs()` exatamente com a finalidade de transformar objetos GTFS que existem apenas dentro do R em arquivos GTFS para arquivos compactados salvos no disco do seu computador. Para esta função, é necessário apenas listar o objeto e o endereço no qual o arquivo deve ser salvo:
...
# cria uma função genérica para filtrar GTFS e plotar figura
...
Esta função genérica criada acima:
...
```{r}
# converte para objeto espacial sf o traçado de um shape_id 
# pega o bounding box desse traçado
bbox <- convert_shapes_to_sf(gtfs, shape_id = "68962") |>
        sf::st_bbox())


# plota toda rede de transporte público e o bounding box construído acima
plotter(gtfs, bbox, do_filter = FALSE)
```

Note aqui que nós usamos a função `convert_shapes_to_sf()`, que converte uma determinada trajetória descrita no GTFS em um objeto espacial do tipo `sf`, para obter a *bounding box* da trajetória. Por padrão, a `filter_by_sf()` (e a `plotter()`, consequentemente) mantém os dados relacionados aos registros de viagens cujas trajetórias possuem alguma interseção com o polígono espacial selecionado: 
