...
Os dados de distribuição espacial da população e de suas características em termos de renda domiciliar *per capita*, cor, sexo e idade, provenientes do Censo 2010 e agregadas pelo projeto Acesso a Oportunidades, podem ser baixados com a função `read_population()`. A função requer que um valor seja passado para o parâmetro `city`, utilizado para indicar a cidade cujos dados devem ser baixados. Para baixar os dados com as informações espaciais de cada célula da grade, o parâmetro `geometry` deve receber o valor `TRUE` (por padrão, esse valor é `FALSE`, fazendo com que a geometria das células não seja baixada).
...
tbl <- tibble::tribble(
  ...
  "R001",   		 "Renda domiciliar *per capita* média", 		    "Valores de 2010 (em R$)",
